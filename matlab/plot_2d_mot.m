...
v_r = @(v) sum(v(:,1).^2+v(:,2).^2, 2).^0.5;
[initial_counts,edges] = histcounts(v_r(initial_velocity), 100);
final_counts = histcounts(v_r(final_velocity), edges);
centres = (edges(1:end-1)+edges(2:end))/2;

plot(centres, initial_counts, 'k-'); hold on;
plot(centres, final_counts, 'b-'); hold off;
