...
impl Laser {
	...
	pub fn create(
		...
		Laser {
			...
			centre: intersection,
			wavenumber: maths::array_multiply(
				&maths::norm(&direction),
				2.0 * constant::PI / (wavelength * 1e-9),
			),
...
impl<'a> System<'a> for CalculateCoolingForcesSystem {
	...
	fn run(&mut self, (pos, laser, vel, mag, mut cooling_force, atom, timestep): Self::SystemData) {
		...
		for laser in (&laser).join() {
			...
			for (vel, _pos, mag, mut cooling_force, atom) in
				(&vel, &pos, &mag, &mut cooling_force, &atom).join()
			{
				let br = mag.magnitude;

				let laser_intensity = 1.0;
				...
				let scatter1 = 0.25 * (laser.polarization * costheta + 1.).powf(2.) * atom.gamma
					/ 2. / (1.
					+ s0 + 4. * (detuning - atom.mup / HBAR * br).powf(2.)
					/ atom.gamma.powf(2.));
				let scatter2 = 0.25 * (laser.polarization * costheta - 1.).powf(2.) * atom.gamma
					/ 2. / (1.
					+ s0 + 4. * (detuning - atom.mum / HBAR * br).powf(2.)
					/ atom.gamma.powf(2.));
				let scatter3 = 0.5 * (1. - costheta.powf(2.)) * atom.gamma
					/ 2. / (1.
					+ s0 + 4. * (detuning - atom.muz / HBAR * br).powf(2.)
					/ atom.gamma.powf(2.));
				...
				cooling_force.force =
					maths::array_addition(&cooling_force.force, &scattering_force);
				cooling_force.total_impulse = cooling_force.total_impulse
					+ maths::modulus(&scattering_force) * timestep.delta;
			}
		}
	}
}
...
impl<'a> System<'a> for CalculateRandomScatteringForceSystem {
	...
	fn run(&mut self, (mut cooling_force, atom, timestep): Self::SystemData) {
		for (mut cooling_force, atom) in (&mut cooling_force, &atom).join() {
			...
			loop {
				if num_kick > 1. {
					...
					num_kick = num_kick - 1.;
					force = maths::array_addition(
						&force,
						&maths::array_multiply(
							&maths::random_direction(),
							momentum_photon / timestep.delta,
						),
					);
				} else {
...
pub fn add_systems_to_dispatch(
	...
	builder
		...
		.with(
			CalculateCoolingForcesSystem,
			"calculate_cooling_forces",
			&["clear_cooling_forces"],
		)
		.with(
			CalculateRandomScatteringForceSystem,
			"calculate_random_scattering_forces_cooling",
			&["calculate_cooling_forces"],
		)
...
pub mod tests {
	...
	fn test_add_cooling_forces() {
		...
		let mut dispatcher = DispatcherBuilder::new()
			.with(AddCoolingForcesSystem, "add_cooling_forces", &[])
			.build();
		dispatcher.setup(&mut test_world.res);
		...
		let force_comp = Force { force: force };

		let test_entity = test_world
			.create_entity()
			.with(cool_force_comp)
			.with(force_comp)
			.build();
		...
		assert_eq!(
			test_world
				...
				.force,
			maths::array_addition(&cool_force, &force)
		);
	}
