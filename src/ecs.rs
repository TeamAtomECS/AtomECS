...
impl AtomecsDispatcherBuilder {
	fn new() -> AtomecsDispatcherBuilder {
		AtomecsDispatcherBuilder {
			builder: DispatcherBuilder::new(),
		}
	}

	fn add_frame_initialisation_systems(&mut self) {
	...
	fn add_integration_systems(&mut self) {
		&self
			.builder
			.add(EulerIntegrationSystem, "euler_integrator", &["add_gravity"]);
	}

	fn add_frame_end_systems(&mut self) {
		&self
			.builder
	...
	fn build(mut self) -> DispatcherBuilder<'static, 'static> {
