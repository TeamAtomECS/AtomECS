...
pub struct MagneticDipoleField {
...
impl Component for MagneticDipoleField {
    type Storage = HashMapStorage<Self>;
}

/// Updates the values of magnetic field samplers to include dipoles in the world.
pub struct SampleMagneticDipoleFieldSystem;

impl SampleMagneticDipoleFieldSystem {
...
impl<'a> System<'a> for SampleMagneticDipoleFieldSystem {
    type SystemData = (
        WriteStorage<'a, MagneticFieldSampler>,
        ReadStorage<'a, Position>,
        ReadStorage<'a, MagneticDipoleField>,
    );
        ...
        for (position, dipole) in (&positions, &dipoles).join() {
            (&positions, &mut sampler)
                ...
                .for_each(|(location, mut sampler)| {
                    let field = SampleMagneticDipoleFieldSystem::calculate_field(
                        location.pos,
                        position.pos,
                        dipole.moment,
...
pub mod tests {
    ...
    fn test_dipole_field() {
        ...
        let field =
            SampleMagneticDipoleFieldSystem::calculate_field(location, position, moment, direction);
